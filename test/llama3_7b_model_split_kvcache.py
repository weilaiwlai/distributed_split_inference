...
def warmup_split_llm(generator, warmup_prompt="Warmup test", max_new_tokens=100, n_warmup_runs=2):
    ...
    with torch.inference_mode(): 
        for i in range(n_warmup_runs):
            print(f"warmup_split_llm {i+1}/{n_warmup_runs}")
...
if __name__ == "__main__":
    ...
    response = generator.generate(prompt="Hey, how are you doing today?", max_new_tokens=20, temperature=1, )
    end_time = time.time()

    elapsed_time = end_time - start_time
    tokens_per_second = 20 / elapsed_time
